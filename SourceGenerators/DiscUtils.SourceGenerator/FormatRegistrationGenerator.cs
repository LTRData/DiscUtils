...
public sealed class FormatRegistrationGenerator : IIncrementalGenerator
{
    ...
    private static string RegistrationNamespace(string assemblyName)
        => string.Join(".", assemblyName.Split('.').Select(segment =>
        {
            ...
            return SyntaxFacts.GetKeywordKind(name) == SyntaxKind.None ? name : "_" + name;
        }));
        ...
        ImmutableArray<Registration> registrations, bool isLibrary)
    {
        ...
        var namespaceName = RegistrationNamespace(assemblyName);
        var setupName = namespaceName + ".Formats";
