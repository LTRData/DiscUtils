...
Managed .NET support for reading XFS filesystems, including file contents and filesystem metadata.
...
```sh
dotnet add package LTRData.DiscUtils.Xfs
```
...
Includes Unix metadata and file allocation extents. The implementation does not format, repair or modify XFS volumes.
...
```csharp
DiscUtils.Xfs.Formats.Register();
```
...
using var stream = File.OpenRead("xfs.img");
using var fs = new XfsFileSystem(stream);
