...
Managed .NET support for reading, creating and modifying fixed, dynamic and differencing Microsoft VHDX virtual disk images.
...
```sh
dotnet add package LTRData.DiscUtils.Vhdx
```
...
```csharp
DiscUtils.Vhdx.Formats.Register();
```
...
using var image = File.Create("new.vhdx");
